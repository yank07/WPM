...
class TestRevivir_item(TestCase):
    ...
    def setUp(self):
        ...
        self.item = Item.objects.create(nombre='item1', tipoitem=self.tipoitem, fase=self.fase, complejidad=10,
                                        costo=10, estado='ELIM')

    def test_revivir_item(self):
        c = Client()
        #iniciamos sesion
        c.login(username='super', password='super')
        url = reverse('item.views.revivir_item', args=[self.item.id])
        response = c.get(url, follow=True)
        estado_item = Item.objects.get(id=self.item.id).estado
        self.assertEqual(estado_item, 'ACT')